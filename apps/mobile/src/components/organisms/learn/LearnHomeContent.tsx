...
/**
 * The Learn home body, without any screen chrome (no SafeAreaView, no AppBar),
 * so it renders both as the standalone `/learn` screen (AT-134, PRD-01
 * FR-48/FR-50/FR-51) and as the Trainings module's Learn tab. Every XP, level
 * and progress number comes from `get_learn_home()`, derived server side (the
 * hook never computes XP), keyed off the player's primary sport
 * (athlete_sports.is_primary, 0060). A player with no primary sport, or a sport
 * with no seeded ladder, sees the FR-48 empty state, never a zero-filled
 * roadmap. Deep links (roadmap, drills, milestones, drill detail) push the
 * `/learn/*` stack, which carries its own back header. Each state fills its
