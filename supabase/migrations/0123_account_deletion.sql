...
--
-- ENFORCEMENT IS NOT NEW CODE. 0120 already built three layers keyed on
...
comment on function public.is_active_user() is
  'SEC-F4 + 0123. False when the calling user is suspended OR has deleted their account. Used as a RESTRICTIVE policy predicate on client-writable tables. Reads the live row rather than a JWT claim so the refusal takes effect on the next request, not the next token refresh.';
...
begin
  ...
  -- 0123. A deleted account must not be able to obtain or refresh a token.
...
comment on function public.delete_my_account() is
  'App Store 5.1.1(v) in-app account deletion. Purges personal data, retains money-bearing and moderation rows against an anonymized users row, and stamps deleted_at so all three 0120 enforcement layers refuse the account. Targets auth.uid() only, never a parameter.';
