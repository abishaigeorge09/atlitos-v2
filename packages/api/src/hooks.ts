...
/** One page of a creator's clip grid.
 *
 * Small on purpose. The profile grid does not just fetch these rows, it mints
 * a signed playback URL for every one of them (profile/index.tsx batches the
 * whole active grid, not the visible cells) and decodes a bitmap per tile. At
 * the measured 1.48 MB average `clips` storage object, an unbounded grid for a
 * creator with 300 clips requested roughly 3.6 GB of decoded bitmap and 13
 * edge function invocations to show 9 tiles (SCALE-CLIENT.md P1-1). Cursor
 * pagination for older pages is a screen change and is not in this pass. */
const PROFILE_GRID_PAGE_SIZE = 30;
  ...
  return {
    ...
    async getMyClips(): Promise<Clip[]> {
      ...
      const { data, error } = await db
        ...
        // PROFILE_GRID_PAGE_SIZE tiles. Worth knowing before someone reads a
