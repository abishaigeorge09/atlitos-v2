...
### Post-P8 QA remediation: UPA money-in visibility + supporters/reviews (0084)

QA audit (`docs/qa/QA-AUDIT-REPORT.md`) #1 backlog item. Founder-greenlit. Fully READ-ONLY inside the financial invariant, no money rows or status writes.

- Root cause (SQL-verified on `syzzfgaudpifwvbpycyi`): money-in was incoherent because per-item and profile surfaces trusted the `upa_wishlist_items.funded_amount` CACHE, which drifted from real donations (cache ₹9,900 vs ledger/donations ₹6,725 for the cricket UPA `4f7616f4`; the "First aid kit" item cached ₹2,500 with 0 attributed donations). The dashboard "Total raised ₹0.00" seen in Walk 1 was the SEPARATE `upa.verified@` application `f0000000...0001`, a duplicate-headline seed row with genuinely zero donations, not the funded coach1@ row. The `upa_fund_balance` RPC was already correct.
- Migration `0084_upa_money_visibility.sql`: new `upa_money_summary(p_upa_id)` (SECURITY DEFINER, scoped owner-or-verified) returning ledger `total_raised`, `donor_count`, per-item DERIVED funded map, supporters, and gratitude; and a rebuilt `public_upa_profile` that derives per-item funding and adds `donor_count`/supporters/gratitude. Grants `anon, authenticated, service_role`.
- SQL proof: `upa_money_summary` total == `upa_fund_balance` == `sum(donations.amount)` (live figure, 6,725 rising to 6,976 as a concurrent donate agent added rows); per-item derived `{5000, 400}`, drifted item drops to 0; donor_count 2; supporters 2; gratitude 1.
