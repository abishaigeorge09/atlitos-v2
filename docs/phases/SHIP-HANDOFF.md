...
## Post-P8 addendum: Trainings fixed shell navigation (2026-07-25)

Founder directive: inside Trainings, switching Stats / Coaches or Trainees / Payments or Earnings / Chat / Analytics must not open a new page each time. The module header and TrainingsSubNav are now FIXED; only the content below swaps, tab switches never grow the back stack, and chat no longer redirects to a different surface.

Architecture:

- `(tabs)/trainings/_layout.tsx` stays the module Stack. Its root is a new `(shell)` route group whose `_layout.tsx` is the persistent shell: SafeArea + "Trainings" title + TrainingsSubNav rendered once, over an expo-router `Tabs` navigator with `tabBar={() => null}` and `backBehavior="none"`. The five per role tab screens (`index`, `coaches`, `trainees`, `payments`, `earnings`, `chat`, `analytics`) are its sibling screens: per tab state preservation, lazy mount, no animation. TrainingsSubNav drives it with `router.navigate` (never push); the active tab derives from the current route segment. Nesting Tabs inside the parent `(tabs)` Tabs works on expo-router 57, no fallback needed.
- Tab screens no longer render SafeArea, AppBar or the sub nav; the shell owns the header. Guests and pending or rejected coaches see the gate or verification status as Stats tab content inside the shell (sub nav hidden for them, as before).
- Chat tab embeds the thread list inline via the new shared organism `components/organisms/chat/ChatThreadList.tsx` (extracted from `(tabs)/chat/index.tsx`, which now wraps the same organism with its Messages header). Tapping a thread pushes `trainings/chat-thread/[id]` (re-export of `chat/[id]`) full screen above the module; back returns to the Chat tab with the shell intact.
- Drill ins stay push screens on the module Stack above the shell: requests, availability, trainee/[id], session/[id], earnings/payout-setup, earnings/transfer, verification, chat-thread/[id], and the new `trainings/booking/[id]` (re-export of `coaching/booking/[id]`). The session probe for a player viewer and the Payments rows now use `trainings/booking/[id]` instead of handing off to the coaching surface, because that hand off left the module and back landed on Home instead of the tab that opened it. `earnings/transfer`'s "Back to earnings" and the trainee detail "Message" button use `router.navigate` back into the shell for the same reason.
- Back semantics: from any tab one back leaves Trainings (inner Tabs `backBehavior="none"`, tab switches add no history); from a drill in, back pops to the tab that opened it.

Real bug found and fixed while verifying: `use-chat.ts` `subscribeToThread`/`subscribeToInbox` reused the client's named channel after `.unsubscribe()`, and `.on("postgres_changes")` on a once subscribed channel throws (crashed the Chat tab on remount, and would have crashed reopening the same thread). Both now drop any stale same name channel before subscribing, and both call sites clean up with `supabase.removeChannel(channel)`.

Verified: bare tsc green; `.maestro/trainings-shell.yaml` (new, signs in as player@atlitos.dev then coach1@atlitos.dev) passes end to end twice in a row: all five tabs per role render their distinctive content with the shell title and sub nav visible at every step, thread open and back, session or booking drill in and back to Stats (player), availability drill in and back to Stats (coach), one Home tap exits cleanly. Full existing guest suite (9 flows) re-run green. Screenshots per tab per role in /tmp/trainings-shell on the build machine. One demo data change: the accepted player session moved to 2026-07-30 so an upcoming session card exists to drill into. Known flake, not app logic: the FIRST bottom nav tap right after login is occasionally swallowed in the simulator, so the flow retries it; and switching accounts three times in one long lived dev process can flake the `getMe` refresh (shell correctly shows the error state with Retry).
