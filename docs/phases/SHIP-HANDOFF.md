...
1. **Native screen coverage** across every prior phase: no screen has been exercised on a real device. Walk all core surfaces at phone width on the simulator.
2. **`react-native-video` autoplay** in the Clutch feed: muted autoplay, poster-to-video swap, next-card prefetch. Mobile-web never exercised the native player.
3. **`react-native-razorpay` native checkout sheet**, including **the failure path** (dismiss vs decline): `Alert.alert`/AT-64 behavior and the booking never rendering confirmed on an abandoned/declined payment. The success path was touched once natively (session `43c52265`); the failure path is unverified.
4. **Camera capture + gallery picker** (`expo-image-picker`) for clip upload and avatar/photo flows.
5. **`expo-haptics`** across the app (confirmations, money actions, feed interactions).
6. **`expo-location`** (court discovery distance, `ai-search` `lat`/`lng` narrowing).
7. **Splash screen** and app-launch experience.
8. **Phone-width layout** on real device dimensions (not the web viewport).
9. **iOS keyboard offsets** on every form (auth, onboarding, checkout address, chat, support).
10. **The `Alert.alert` / AT-64 call sites** (10 money-consequential confirms) and their `ConfirmSheet` replacements: verify each fires and blocks natively. `Alert.alert` is inert on react-native-web, so these were never provable before.
11. **Device push transport for notifications**: exercise `notify-dispatch`'s stubbed `deliverToDevice()` seam with real APNs/FCM credentials. P8 shipped only the in-app `notifications` row write + in-app surface; the device leg is P9's to wire and prove.
12. **Native-theme verification**: the mobile-web dark-mode gap (real routes did not follow OS dark) becomes a native light/dark theme pass on device. `packages/theme` tokens must resolve correctly under both.
