...
| `renewMembership` | `renew-group-membership` edge function | `POST { membership_id, expected_total }`, same response shape; `active` or `expired` memberships (`0090`: an expired member still holds their seat and renews on the same row, a lapsed member re-joins because their seat was released); re-snapshots the fare at today's price |
...
Two SQL emitters were added alongside it and, like the RPCs listed above, write the `notifications` row directly rather than routing through this function (the device push leg is still the P9 stub, so the HTTP hop from inside a transaction would buy nothing today):

- `notify_session_parties(p_session_id uuid, p_action text) returns int` — `service_role` only (`0089`). Called by `session_transition_internal` on `accept`, `decline`, `start` and `complete`. Writes one `session` type notification per athlete party (the 1:1 `player_id`, or every `session_participants` row for a group session) and never to the acting coach. Returns the number written.
- `sweep_group_memberships() returns jsonb` — `service_role` only (`0090`), scheduled as `membership-sweep` daily. Writes `membership` type notifications for the renewal reminder, expiry and lapse. See SCHEMA.md "The membership sweep".
