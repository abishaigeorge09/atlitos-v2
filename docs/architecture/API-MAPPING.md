...
Two SQL emitters were added alongside `notify-dispatch` and, like the RPCs listed in the table above (`moderate_clip`, `record_donation_from_draft`, the verification RPCs), write the `notifications` row directly rather than routing through that function (the device push leg is still the P9 stub, so the HTTP hop from inside a transaction would buy nothing today):
