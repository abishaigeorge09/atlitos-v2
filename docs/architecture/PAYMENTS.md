...
**As built (P6, Track B, AT-111 to AT-114).** `donate` creates ONLY the `payment_intents` row (`domain='donation'`, `entity_id=NULL`) plus a `donation_drafts` row (`0053`, the `order_drafts` analog) staging `{upa_id, item_id, donor_id, amount}`, then the Razorpay order. The `donations` row does not exist until capture. It also honours an optional `expected_total`: a mismatch with the server-rounded `amount` is `PRICE_MISMATCH` (422 for `MIN_AMOUNT`, 409 for `PRICE_MISMATCH`/`ITEM_FUNDED`). On capture, `finalize-donation-payment.ts` is the fourth branch of the shared gate (before the null-`entity_id` check, like commerce): it calls `record_donation_from_draft` (`0054`, atomic: inserts the `donations` row, increments `funded_amount` under a row lock, flips `open -> funded` when reached, notifies the UPA owner, idempotent on `payment_intent_id`), backfills `payment_intents.entity_id`, then writes the balanced group `debit platform / credit upa_fund(<upa_id>)` with NO fee leg (PRD-06 FR-6). A general (no item) donation still credits the UPA's own `account_ref`. The `ITEM_FUNDED` guard lives at the `donate` request layer; finalize never rejects a captured donation (it always records it and increments `funded_amount`, so a race overfunds rather than losing money).

**Donor name snapshot (AT-149, migration `0067`, PRD-05 FR-17 / PRD-06 anonymization).** `record_donation_from_draft` snapshots `donations.donor_display_name` in the SAME transaction as the row insert: `users.name` if `users.show_donor_name` was true at that moment, else null. The UPA-facing sponsor list (`portal-life` funding detail) reads this column and never the live `users` row, so it respects the opt-in AND cannot leak the name of a donor who later toggles off, matching the order address-snapshot discipline. This is a display-only add: the amount, the `funded_amount` increment, the `open -> funded` flip, the owner notification, and the balanced ledger group are all unchanged, and the snapshot lives in the RPC (finalize's atomic state half, the FR-9 atomicity precedent) rather than a second edge-function write, so `finalize-donation-payment` itself needed no code change. Verified against the live stack: a scripted `show_donor_name=true` donation snapshots the name, a `false` one snapshots null, both inside a rolled-back transaction; clients still cannot write `donations` at all (single `donations_select_merged` SELECT policy, no insert/update/delete policy, so any client write is `42501`).

**The roundup allocation is live, both arms.** Forward: `finalize-order-payment.ts` now credits the roundup leg to `upa_fund` at the General Fund anchor `general_fund_account_ref()` (TS mirror `GENERAL_FUND_ACCOUNT_REF = 00000000-0000-4000-a000-0000000f0000`) inside the order's own commerce group, and writes a `method='checkout_roundup'` `donations` row (`upa_id` NULL, `order_id` set) in the same delivery; no roundup ever parks on `platform` again. Backfill: migration `0055` moved every historical `platform` roundup leg to the General Fund via a balanced reclass group (`debit platform / credit upa_fund(general)`) plus its `donations` row, idempotent on one `checkout_roundup` donation per order. Verified on the deployed stack (`scripts/verify-empower-p6.mjs`): platform nets `0.00` over all roundup legs, the General Fund balance equals the sum of every `checkout_roundup` donation.
...
**Shop needs no Atlitos payment processing (WS4 affiliate pivot).** With Shop moving to an affiliate model, a shop purchase leaves the app to the merchant's own checkout; Atlitos originates no Razorpay order, writes no `payment_intents`/`ledger_entries` row, and holds no funds for a shop purchase. The `checkout` edge function and its BillSummary remain documented below for the domains that still charge in-app (sessions, courts, donations), but the commerce charge path is not part of the affiliate shop flow. This is a positioning note; existing commerce payment code is retained, not removed, under WS2.
...
### UPA disbursement: the same Route blocker, no transfer built

The Atlitos Life (UPA) portal shows a verified athlete a "Money in and where it goes" panel (0084, QA money-in remediation): the ledger-derived total raised for them and an honest "payouts arriving soon" state. This is a VISIBILITY seam only. It deliberately builds no transfer and invents no disbursed figure, for the same reason coach and court payouts cannot move money: **Route is not enabled on the test merchant** (see the blockers above). There is no `payout_accounts` row type for a UPA yet (`owner_type` today is `coach` / `court_partner` only), no onboarding, and no `razorpay-route-transfer` path for the empower domain. When Route is turned on in the Razorpay dashboard, the UPA payout path would follow the exact shape AT-42/AT-43 built for coaches: a `razorpay-route-onboard` `owner_type='upa'` extension, the `active` gate, then on-demand `record_transfer` writing the debit-the-upa-fund / credit-platform group under the service role. Until then the panel stays at "arriving soon" and writes nothing. The money-in read itself is fully live: `upa_money_summary` and `public_upa_profile` derive every figure from `donations`/`ledger_entries` server-side (SECURITY DEFINER, scoped owner-or-verified), never from the `upa_wishlist_items.funded_amount` cache, which the QA audit found drifted (cache ₹9,900 vs ledger ₹6,725 for the cricket UPA).

## Membership fares (training groups, the fifth domain)

Founder-ratified model: monthly subscription per group, MANUAL renewal in v1 through these same one-time rails (no autopay, no saved instruments), no-show has no money effect, no pro-rata refunds, capacity guarded inside the join RPC.

The join leg mirrors book-session byte for byte: `join-group` re-prices server side from `training_groups.monthly_fee` plus the active `sessions.platform_fee_flat` row (the SAME carve-out shape as sessions: total = monthly_fee, the fee comes out of the coach's amount, `PRICE_MISMATCH` on a stale client total), then calls the service_role-only RPC `join_training_group`, which locks the group row, counts live (pending + active) memberships against capacity, and inserts the membership as `pending` with the fare snapshotted onto it. `GROUP_FULL` / `ALREADY_MEMBER` / `GROUP_INACTIVE` are refused BEFORE Razorpay is called, so a losing racer is never charged; a Razorpay failure afterwards releases the seat via `membership_abandon_unpaid` (pending -> lapsed), never a raw status write. Then payment_intents (domain `membership`, entity_id = membership id), the Razorpay order, the order id backfill, and the intent link, in the book-session order.

The capture is the money event, unlike sessions (which accrue at completion, because a paid session can still be declined): a bought month is earned when it is bought. `_shared/finalize-payment.ts` gained the membership branch, and `_shared/finalize-membership-payment.ts` does two things once per captured charge: `activate_group_membership_paid` (pending -> active, period_start = today IST, period_end = +1 month; on a renewal intent it extends period_end by 1 month from max(period_end, today)), then the balanced group under the service role:

    debit  platform            monthly_fee        clearing
    credit coach <coach_id>    monthly_fee - fee  membership earnings
    credit platform            fee                platform fee

Renewal (`renew-group-membership`) is the same rails against the SAME membership row: the RPC `renew_group_membership` re-snapshots the fare at today's monthly_fee + fee_config (an admin fee edit applies to the next month, never retroactively), a NEW intent is created and linked, and its capture writes its own ledger group, so two payments are two balanced groups. A lapsed member does not renew; they re-join through the capacity guard, because their seat was already released. Group SESSION rows carry no money at all (price 0, no intent, no accrual): completing a group session moves nothing, which is why 0077 lets group sessions complete through the client `session_transition` door while 1:1 completion stays locked to complete-session.
...
There are two KINDS of refund path in v2, and the distinction is deliberate: some require human judgement, some provably do not. The automatic kind now has two members, one per party who can dispose of an unanswered, already-paid coaching request: the athlete (`cancel-session-refund`) and the coach (`decline-session-refund`). Both are the same money event pointed at the same payer, and both reuse one machinery.
...
**`decline-session-refund` (added 2026-07-27, QA finding CO-04, PRD-02 FR-14 + FR-35).** The coach-side mirror of the above, and the fix for a P0 money hole the QA suite found: a coach declining a `requested` session whose `payment_intents` row was already `captured` left `status='captured'` with no `refunds` row and no reversing group. A session accrues NO ledger group at capture (`finalize-session-payment.ts`: the coach is credited only at completion), so a declined session correctly has zero ledger rows either way, which masked the hole: the captured charge was never reversed and the athlete's money was taken and never returned. The economic case is identical to the athlete-cancel case (no service rendered, no fee accrued, full amount back), so the fix reuses the SAME machinery rather than inventing a second: the function calls `session_transition_internal(coach, id, 'decline', reason)` under the service role, then claims a `refunds` row keyed on `(domain='session', entity_id)`, calls Razorpay's refund API, and settles through `settle_refund` (the reversing `debit platform / credit user <payer>` group). It is idempotent and admin-visible on failure exactly as `cancel-session-refund` is. The unskippability is enforced the AT-61 way (migration `0085`): `session_transition('decline')` now raises `USE_EDGE_FUNCTION` for `authenticated` callers, so the edge function is the only decline path and the refund cannot be routed around. A `requested` session with no captured intent (an abandoned-payment case, `0024`) is declined with `refund_status: not_applicable` and no `refunds` row. Proven end to end against the deployed backend (`scripts/verify-co04-decline-refund.mjs`): a captured 1000 request declined through the function leaves the session `declined`, the reversing group `debit platform 1000 / credit user 1000` nets `domain='session'` for that entity to `0.00`, `payment_intents.status` reaches `refunded`, and the bare RPC is refused with `USE_EDGE_FUNCTION` without moving the session. Against the synthetic capture rail the Razorpay refund leg cannot settle (no real payment behind it), so the settlement was driven through `settle_refund`, the same convergence point the `refund.processed` webhook calls; on a real Razorpay payment the function reaches it itself, exactly as `cancel-session-refund` did for session `43c52265`.
...
Worked example, courts. `court_booking_confirm_payment` succeeds so the booking reaches `confirmed`, then the `ledger_entries` insert fails. The partner is never credited, the balanced group for that money never exists, and `expire_stale_holds()` did not look at captured intents at all.

**As built.** `payment_intents.finalized_at` records that the domain handler completed. The gate now has three outcomes on a zero-row flip rather than two: no such intent (`NOT_FOUND`), a `captured` intent with `finalized_at is null` (resume it, re-entering the handler), or a genuinely complete charge (`already_processed`). Resume is scoped strictly to `captured`; a `refunded` or `partially_refunded` intent has moved past capture and must not have its capture-time handler re-run.

Re-entry is safe because every handler was already idempotent. The domain RPCs (`place_order_from_draft`, `record_donation_from_draft`, `activate_group_membership_paid`, `court_booking_confirm_payment`) return the existing row untouched on a repeat, and the commerce, donation and membership handlers already guarded their ledger write with an existence check. Courts was the one handler missing that guard and gained one in the same change.

**Why no unique index on `ledger_entries`.** The obvious backstop, a unique index over `(payment_intent_id, account_type, direction, domain, entity_id)`, cannot be used: `settle_refund` writes a reversing group against the same `payment_intent_id` whose legs deliberately collide with the capture group's on exactly those columns. The capture group is always the FIRST ledger group an intent ever has, so "any ledger row exists for this `payment_intent_id`" is the correct and sufficient idempotency predicate, and that is what the handlers use.

**What is still owed.** Repair happens on the next delivery of a capture. `payment_finalization_backlog()` and the `payments_unfinalized` arm of `expire_stale_holds()` make a stuck charge visible, but nothing re-invokes finalization on its own. A scheduled worker that re-calls `verify-payment` per backlog row is the upgrade path; it is deliberately not built until the backlog is observed to be non-transient.

The public `outcome` contract is unchanged. A resumed finalization returns `"captured"`, not a third value: from every caller's point of view the charge is now complete, and widening `"captured" | "already_processed"` would ripple through eight typed client call sites for an observability nicety that the server log and `finalized_at` already carry.
