...
**`ON DELETE` behaviour of everything pointing here changed in `0107`.** `ledger_entries.payment_intent_id`, `sessions.payment_intent_id` and `group_memberships.payment_intent_id` were `SET NULL` and are now `RESTRICT`. A captured payment must never become unattributable, and blanking the link is the same loss of attribution as deleting the row. Note the interaction with account deletion: `payment_intents.user_id` cascades from `users`, so deleting a user who has ever paid now FAILS on the ledger's refusal. That is the invariant working; whoever lands `0098` has to choose anonymise-and-retain rather than reverting the edge.
