...
- `training_groups` — id, coach_id -> coach_profiles(user_id), name, sport (the shared `sport` enum), skill_level text, capacity int (check > 0 and, from `0109`, check <= 100), monthly_fee numeric(12,2) (rupees, the exact unit `sessions.price` uses; check >= 0), attendance_policy text, active bool default true, timestamps. Writes are RPC-only: `create_training_group` / `update_training_group` (0080, coach-owned; deactivate, never delete; capacity may not drop below the live member count).
