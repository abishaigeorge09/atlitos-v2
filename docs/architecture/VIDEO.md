...
- **`stream-upload-url`** (POST, athlete JWT, verify_jwt on). Body `{ caption, sport, clip_id? }` (`sport` in the `public.sport` enum: football, cricket, badminton, tennis; `clip_id` optional to reuse an own `uploading` row on a retry). Creates/updates the caller's own clip row at `status='uploading'`, persists `storage_path`/`playback_id = <owner_id>/<clip_id>.mp4`, and returns `{ clipId, uploadUrl, token, path, bucket, status, thumbUploadUrl, thumbToken, thumbPath }`. The client (or a verification script, no device required) PUTs the MP4 straight to `uploadUrl`, or uses supabase-js `uploadToSignedUrl(path, token, file)`. `cf_stream_uid` stays null (future Cloudflare slot).

  **The thumb slot (SCALE-MEDIA M-3, 2026-08-14).** The three `thumb*` fields are a SECOND signed upload URL for `<owner_id>/<clip_id>.jpg`, mirroring the video path. They exist because the `clips` bucket is private and grants clients no storage write (0042), so there was previously no way at all for a client-captured poster to reach storage, and `thumb_path` was NULL for every clip ever posted through the app. All three are nullable: a failed thumb mint logs and the clip posts without a poster, never an error.
- **`stream-webhook`** (POST, uploader JWT, verify_jwt on). The v1 synchronous finalizer (no Cloudflare webhook to lose). Body `{ clip_id, thumb_path? }`. Verifies the object landed, then drives the owner's clip `uploading -> processing -> ready` via `clip_transition_internal`. **`thumb_path` is validated, not trusted (SCALE-MEDIA M-3, 2026-08-14):** it must be exactly `<owner_id>/<clip_id>.jpg`, the one slot `stream-upload-url` minted, and the object must actually exist, or it is refused (400) / ignored respectively. Before that guard any string was written onto the row, so an owner could point `thumb_path` at another object in the private `clips` bucket and have the playback mint sign it for them. Idempotent exactly like `razorpay-webhook`: only acts on a clip still in its pre-state, so a redelivery of an already-`ready` (or terminal) clip is a no-op returning `{ clipId, status, outcome:'already_finalized' }`; a first finalize returns `outcome:'finalized'`.
...
**LAUNCH Phase 3, Track B (P1-1, P1-5's rate-limit half; PHASE-3-STATUS.md CT-1, CT-2).** `get-clip-playback-url` gained a BATCH body in place, per the settled decision not to add a second unthrottled front door: `POST { clip_ids: string[] (max 24), kind?: "video" | "thumb" }` returns `{ urls: [{ clip_id, url, expires_at }], failed: [{ clip_id, reason }] }`, 200 even on partial failure. Same live-row authz per id as the legacy single-`clip_id` body (unchanged, still works). TTL split by `kind`: `video` stays 300s; a NEW `thumb` TTL is 3600s (posters are cheap to over-cache; a longer-lived thumb URL means the client re-mints far less often for a scrolling grid). A per-IP token bucket (`clip-playback-ip`, 60 requests/60s, Postgres-backed `take_rate_limit_token`, CT-2) sits in front of BOTH bodies; over it, `429 { "error": "RATE_LIMITED", "retry_after_seconds" }`. The limiter FAILS OPEN on its own RPC error (serves the request, logs, never 500s this public read path); the authz above is untouched by any of this. A second deployed function, `get-clip-playback-urls`, delegates to the identical handler (`supabase/functions/get-clip-playback-url/handler.ts`) so there is exactly one rate-limited code path regardless of which name a caller hits. Implementation: `handler.ts` (shared logic), `mint.ts` (the `thumb`-TTL signed-URL mint, local to this function's directory so the shared `_shared/clip-access.ts` other functions depend on stays untouched), `../_shared/rate-limit.ts` (the token-bucket wrapper). Load harness: `scripts/load/phase3-frontdoor.mjs` (sessionless no-429), `scripts/load/phase3-feed.mjs` (batch-in-one-call + TTLs + forbidden-id-in-batch + concurrent grid loads). Both scripts require a deployed target (`PHASE3_BASE_URL` etc, see each file's header) and were NOT run against a live deployment as part of this track's build; see `PHASE-3-STATUS.md` for the deploy/verify handoff.

**SCALE-MEDIA remediation, 2026-08-14 (M-1, M-3, M-4, M-6). Requires a redeploy of `stream-upload-url`, `stream-webhook`, `get-clip-playback-url` and `get-clip-playback-urls`.**

1. **The throttle is re-keyed.** The single per-IP bucket above became the P0 at 10,000 users. Live `edge_rate_limits` rows under `clip-playback-ip` are Indian carrier CGNAT addresses (`223.228.114.206` and `223.228.125.206` share a /16), so real users share one key and, at a mixed 23 requests/minute/user, THREE concurrent users behind one egress IP exhaust a 60/60s window, while global aggregate load is only about 3.7 requests/second. There are now TWO buckets: `clip-playback-user` 60/60s keyed on the VERIFIED user id for a signed-in caller, and `clip-playback-ip` 600/60s for anonymous callers, sized for CGNAT rather than for one person. The caller is resolved once via `getOptionalUserId` before the throttle and handed to both bodies, so the batch path and the owner/admin legacy path each make one fewer round trip than before. The key is never a locally decoded `sub`: an unverified id would let a forged token mint a fresh bucket and remove the limit. Abuse protection is unchanged in kind, only in key.
2. **The 429 is now legible to the client.** `mapEdgeFunctionError` reads the flat `{ error: "RATE_LIMITED", retry_after_seconds }` body, which it previously could not (it only understood the `{ error: { code, message } }` envelope, so every 429 reached callers as an indistinguishable `INTERNAL` 500). `ApiError` gained an optional `retryAfterSeconds`.
3. **The feed handles a failed mint.** `mintPlayback`'s catch in `(tabs)/clutch/index.tsx` was empty and its refresh timer armed only after success, so one failure left a card black forever with no poster, no error and no retry. It now backs off (four attempts, 2s base doubling to a 30s cap, up to 50 percent jitter, a `RATE_LIMITED` refusal waiting out the server's own window) and shows a non blocking state with a Retry on the card.
4. **Both profile grids use the batch endpoint.** `getPlaybackUrls` had exactly one call site; `ClutchProfileView`, which backs the own Clutch profile AND every creator profile, passed no poster and rendered blank tiles. Both now go through the shared `useClipPosters` hook. `getCreatorClips` and `getMyClips` were unbounded and are now limited to `CLUTCH_GRID_PAGE_SIZE = 24`, deliberately equal to the batch chunk size so a full grid page is exactly ONE request, with an optional `created_at` cursor for the next page.

   **Continuation wired, 2026-08-14 (Track 3).** The bound above shipped WITHOUT a continuation: neither grid screen passed a cursor and neither wired `onEndReached`, so a creator with more than 24 published clips silently lost the rest. Both functions now return `ClipPage { items, nextCursor }` rather than a bare `Clip[]`, decided server side from a `limit + 1` probe, and the cursor is a keyset over `(created_at desc, id desc)` rather than `created_at` alone (a shared timestamp across a page boundary would drop a row invisibly, the same class of loss). `nextCursor` is deliberately derived from the RAW page, not from `items.length`, because `getMyClips` drops soft deleted rows after the limit, so a short page is not proof of exhaustion. Call sites: `(tabs)/clutch/profile.tsx`, `(tabs)/clutch/creator/[id].tsx` and `profile/index.tsx` (posts tab), all through `ClutchProfileView`'s `onEndReached`/`loadingMore`/`hasMore` props or their own FlatList footer.

   **Poster loading is now distinguishable from poster failure.** `useClipPosters` also returns `pendingPosterIds`, and a thumb tile whose mint is in flight renders a `Skeleton` instead of the flat surface it shared with "this clip has no poster". The mint takes about three seconds on a full grid, and on device those blank tiles were nearly filed as the batch endpoint failing. The hook's batch call also gained the rejection handler it never had, so an offline or 429 batch no longer surfaces as an unhandled promise rejection. `profile/index.tsx` had its own hand rolled copy of the mint effect, which is why the bug existed twice; it now shares the hook.
5. **Images are requested at the size they are painted.** New `sizedImageUrl` helper in `@atlitos/api` rewrites a Supabase public object URL onto the `render/image/public` transform endpoint. Applied in the mobile `Avatar` (covering every avatar render site at once), promo banners, product media, venue photos, and the three portal-court venue thumbnails (which use `getPublicUrl`'s native `transform` option, as portal-court does not depend on `@atlitos/api`). Measured against the one real uploaded avatar in production: 298,490 bytes at origin, 8,480 bytes at 240 px q70, a 35x reduction, both HTTP 200.
