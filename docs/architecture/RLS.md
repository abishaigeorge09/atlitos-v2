...
Every access is a security definer function that authorises itself: owners through
`upsert_my_payout_method` and `get_my_payout_method` (ownership re-derived from `auth.uid()`),
admins through the `admin_` payout functions (`has_role('admin')`, every full reveal audited).
...
| `retailer_programmes` (Phase S2) | `has_role('admin')` only (`retailer_programmes_select_admin`); **no anon/authenticated grant at all**, deliberately NOT public-browse like `affiliate_products`: operational config, not shopper content (ADR-011 D6) | **no client write**; `service_role` only. No upsert RPC yet, service role / seed migration. `0123_gear_ingest_health.sql` |
| `product_fetch_log` (Phase S2) | `has_role('admin')` only (`product_fetch_log_select_admin`); no anon/authenticated grant | **no client write**; `service_role` only (`gear-recheck`). `0123_gear_ingest_health.sql`, ADR-011 D4, D6 |
