...
- **P3 Coaching + Chat + Payouts:** sessions engine, Realtime chat, coach earnings, Route onboard+transfer on completion · GATE (amended 2026-07-20, see below): v1 Journeys 2+3 with test payments, and Route transfer proven as far as the account permits

  **Gate amendment, founder decision 2026-07-20.** The original clause required a *visible Route transfer*. Razorpay Route is not enabled on the test merchant account: `POST /v2/accounts` returns 400 "Route feature not enabled for the merchant", and enabling it requires activation work on the account that is not worth blocking six remaining phases behind. Live transfers are needed for production regardless, so this is deferred, not abandoned. P3 now passes on: Journeys 2 and 3 complete with real test payments; the Route onboarding and transfer functions built, deployed, and proven to handle the real API's not-entitled response correctly rather than faking success; and the coach earnings ledger correct and derivable. The live transfer becomes a P8 hardening item, tracked, and must run before any real money moves.
