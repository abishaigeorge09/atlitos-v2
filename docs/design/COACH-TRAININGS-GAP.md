...
| 8 | Player Profile, Video Analytics tab (`1047:16588`) | Empty state "No Data Found", upload FAB | Track F: content built as the `TraineeVideoAnalytics` organism (`apps/mobile/src/components/organisms/trainings/TraineeVideoAnalytics.tsx`), matching the "No data found" empty state; not yet MOUNTED on a tab, no trainee profile tab set exists to hang it off yet (Track C scope). Schema/storage/edge functions are WRITTEN NOT APPLIED/DEPLOYED (`0082_coach_trainee_videos.sql`, `coach-trainee-video-upload-url`, `get-coach-trainee-video-url`); integrator must apply + deploy before this organism has real data. | `screenshot-1784979804451-31.jpg` |
