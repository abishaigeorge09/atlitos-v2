...
- **FR-25**: A session's status only ever transitions along: requested to accepted, declined, or cancelled; accepted to completed, cancelled, or rescheduled; completed to rated. Any other transition attempt from this app is rejected, not silently allowed. (The `requested` to `cancelled` edge was added 2026-07-19; see the PRD-02 FR-19 amendment and PRD-02 FR-34/FR-35 for the athlete-only rule and the automatic full refund.)
- **FR-26**: Cancel, reschedule, and rate on a session are available only in the states the state machine permits (e.g. rate only after completed, only once). Cancel is available in two distinct states and the copy must not conflate them: from `requested` it is a self-serve exit before the coach has answered, refunded in full automatically (PRD-02 FR-35); from `accepted` it is cancelling a commitment the coach has already made, and carries no automatic refund in v1.
