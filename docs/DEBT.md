...
- **`pnpm turbo typecheck` passed while a bare `tsc` in `apps/mobile` failed (three errors).** The turbo task and the bare compiler disagree; find why (cache key, project references, or a different tsconfig) and make the turbo task fail when `tsc` does. Until then the pre-push gate runs bare `tsc` per package. Owner: Prasanth.
