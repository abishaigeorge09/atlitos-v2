...
## SEC — Cross-cutting security regressions (7 cases)

Adversarial cases, driven at the edge functions and the policies the way an attacker would rather than the way the app does. Every case that asserts a refusal first proves its two parties are distinct users, and every RLS case matches the refusal MESSAGE rather than SQLSTATE alone, so a missing grant cannot pass as an enforced policy. Both disciplines come from CLAUDE.md's third RLS incident.

Two lanes, deliberately:

- `apps/e2e/specs/security.spec.ts` (Playwright, `athlete-web` project only so it runs once) exercises the DEPLOYED system: edge functions, signed URLs, live data. Needs `E2E=1` and `SUPABASE_SERVICE_ROLE_KEY`.
- `scripts/verify-security-fixes.sql` exercises the SQL half with NO credentials at all, against any Postgres carrying the migration chain. `./scripts/verify-migrations-local.sh` stands one up from scratch, applies all 91 migrations and runs it. This lane is what makes the migrations provable on a laptop and in CI; it is also what caught the stray tool-call XML at the end of `0027`, which made the chain unreplayable.

| ID | Surface | Persona | Preconditions | Steps | Expected | Lane | Priority | Status |
|---|---|---|---|---|---|---|---|---|
| SEC-01 | supabase/functions/stream-webhook | coach1@ vs any other clip owner | at least one published clip owned by another user, with a storage_path | Mint an own upload ticket, land placeholder bytes, then finalize with thumb_path set to the victim's object path; repeat with a `../` traversal out of the own-folder prefix | Both calls refused 403 FORBIDDEN; clips.thumb_path stays null. Without the guard the path is written verbatim and both signed-URL mints hand it to storage under the service role, exposing every object in the private clips bucket | SQL | P0 | TODO |
| SEC-02 | supabase/functions/get-clip-playback-url | coach1@ vs any other clip owner | same as SEC-01 | Plant the cross-owner thumb_path, finalize legitimately, then request playback on the attacker's OWN clip (the owner branch always grants) | thumbUrl is null and the minted video URL is under the attacker's own folder only; no signed URL is ever produced for another owner's object | SQL | P0 | TODO |
| SEC-03 | supabase/migrations/0089 (RLS) | coach1@ vs a non-trainee | a coach with at least one trainee session, and one signed-in user who is not a trainee | Direct INSERT into coach_trainee_videos with (a) a chosen storage_path, (b) a non-trainee player_id, (c) caption-only for a real trainee | (a) and (b) refused by the row-level security policy, asserted on the message not just SQLSTATE; (c) still succeeds, so the guard did not break the feature | SQL | P0 | PASS (local) |
| SEC-04 | supabase/functions/get-coach-trainee-video-url | coach1@ | a published clip owned by another user | Seed a poisoned coach_trainee_videos row through the service role, then request its signed URL as the owning coach | 403; the mint re-derives coach-videos/{coach_id}/{player_id}/ and refuses anything outside it, including rows written before 0089 | SQL | P0 | TODO (needs live project) |
| SEC-05 | supabase/migrations/0088 + finalize-payment.ts | n/a | migration chain applied | Seed captured intents with and without finalized_at; call payment_finalization_backlog() at several grace windows; call expire_stale_holds(); sweep live captured intents for missing ledger groups | Only the captured-and-unfinalized intent outside the grace window is backlog; has_ledger_group separates work-missing from marker-missing; no live captured charge in court/commerce/donation/membership lacks a ledger group | SQL | P0 | PASS (local) |
| SEC-06 | supabase/migrations/0090 + _shared/supabase.ts | throwaway account + admin@ | admin role present | Suspend via admin_suspend_user, then on the SAME token call a protected edge function, attempt a direct insert, and invoke the token hook; re-suspend; reinstate | 403 ACCOUNT_SUSPENDED from the edge layer, RLS refusal on the direct write, a 403 error object from the hook; exactly one audit row and one notification per action; re-suspend writes nothing; reinstate restores access | SQL | P0 | PASS (local) |
| SEC-07 | supabase/migrations/0091 | admin@ | an order in placed | Advance an order, attempt an illegal skip, and cancel a second order straight from placed | One audit row per accepted advance with the observed before status, none for the refused edge, and placed -> cancelled records placed rather than the in_transit the pre-0091 lookup fabricated | SQL | P1 | PASS (local) |
